## Create requests in Postman.

### Protocol: ```http```
### IP: ```162.55.220.72```
### Port: ```5005```


## 1. Get request:
![img_6.png](/HW_1_Screenshot/img_6.png) 
#### Create new collection through ```+``` with name ```HW 1```
#### In the current collection, create requests in Postman, click on ``` add a request``` and name``` get request```
#### Choose ```get``` method
#### In the ```Enter URL or paste text field```, enter: 
#### ```Protocol, IP, Port and EndPoint```
#### In ```Params``` enter: ```name and age```
![img_1.png](/HW_1_Screenshot/img_1.png) 

## 2. Post request:
![img_3.png](/HW_1_Screenshot/img_3.png) 

#### Click on ```Duplicate``` 
![img_4.png](/HW_1_Screenshot/img_4.png) 
#### Change the request name to ```post request```
#### Choose ```post```method
#### In the ```Enter URL or paste text field```, enter: ```Protocol, IP, Port and EndPoint```
#### In `form-date` enter data as documentation
![img_5.png](/HW_1_Screenshot/img_5.png) 

## 3. Get method, EndPoint: `/object_info_1`
![photo_12.jpg](/HW_1_Screenshot/photo_12.jpg) 
#### Click on `...` then `add request`
#### Choose `Get` method
#### In the `Enter URL or paste text field`, enter: `Protocol, IP, Port and EndPoint`
### In `Params` enter data as documentation
![Screenshot48.png](/HW_1_Screenshot/Screenshot48.png) 

## 4. Method: GET, EndPoint: `/object_info_2`
![52.png](/HW_1_Screenshot/52.png) 

#### Click on `Duplicate`
#### In the `Enter URL or paste text field`, enter: `Protocol, IP, Port and EndPoint`
### In `Params` enter data as documentation
![33.png](/HW_1_Screenshot/33.png)

## 5. Method: GET, EndPoint: `/object_info_3`


![28.png](/HW_1_Screenshot/28.png) 
#### Click on `Duplicate`
#### In the `Enter URL or paste text field`, enter: `Protocol, IP, Port and EndPoint`
### In  `Params` enter data as documentation
![16.png](/HW_1_Screenshot/16.png) 
![4.png](/HW_1_Screenshot/4.png) 

## 6. Method: GET, EndPoint: `/object_info_4`

![6.png](/HW_1_Screenshot/6.png) 
#### Click on `Duplicate`
#### In the `Enter URL or paste text field`, enter: `Protocol, IP, Port and EndPoint`
### In `Params` enter data as documentation
![5.png](/HW_1_Screenshot/5.png) 

## 7. Method: POST, EndPoint: `/user_info_2`
![2.png](/HW_1_Screenshot/2.png)

#### Click on `...` then `add request`
#### Choose `POST` method
#### In the `Enter URL or paste text field`, enter: `Protocol, IP, Port and EndPoint`
### In `Params` enter data as documentation

![17.png](/HW_1_Screenshot/17.png)  



